package com.company.foodtrucks;

import com.company.foodtrucks.model.FoodTruck;
...
public class Application implements CommandLineRunner {
    ...
    public void run(String[] args) throws IOException, SodaError, InterruptedException {
        ...
        final String now = LocalTime.now().format(FoodTruck.FORMATTER);
        ...
        SoqlQuery departmentOfStateQuery = new SoqlQueryBuilder()
                ...
                .setWhereClause(String.format("start24 <= '%s' AND end24 >= '%s' AND dayofweekstr = '%s'", now, now, dayOfWeek))
                .addOrderByPhrase(new OrderByClause(SortOrder.Ascending, "applicant"))
                .build();

        final List<FoodTruck> foodTrucks = consumer.query("jjew-r69b", departmentOfStateQuery, FoodTruck.LIST_TYPE);
